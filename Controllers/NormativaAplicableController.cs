...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class NormativaAplicableController : ControllerBase
    {
        ...
        public NormativaAplicableController(INormativaAplicableService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] NormativasAplicables m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] NormativasAplicables m)
