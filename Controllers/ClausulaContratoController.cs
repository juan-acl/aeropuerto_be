...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class ClausulaContratoController : ControllerBase
    {
        ...
        public ClausulaContratoController(IClausulaContratoService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] Contratos m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] Contratos m)
