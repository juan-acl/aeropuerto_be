...
namespace Aeropuerto.Backend.Controllers
{
    ...
    {
        private readonly IAsignacionServicioService _service;
        public AsignacionServicioController(IAsignacionServicioService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] AsignacionServiciosTransporte m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] AsignacionServiciosTransporte m)
