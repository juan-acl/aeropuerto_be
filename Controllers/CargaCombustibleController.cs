...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class CargaCombustibleController : ControllerBase
    {
        ...
        public CargaCombustibleController(ICargaCombustibleService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] CargasCombustible m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] CargasCombustible m)
