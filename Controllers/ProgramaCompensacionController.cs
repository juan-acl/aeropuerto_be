...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class ProgramaCompensacionController : ControllerBase
    {
        private readonly IProgramaCompensacionService _service;
        public ProgramaCompensacionController(IProgramaCompensacionService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] ProgramasCompensacionAmbiental m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ProgramasCompensacionAmbiental m)
