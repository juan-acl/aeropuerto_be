...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class SimulacroController : ControllerBase
    {
        ...
        public SimulacroController(ISimulacroService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] Simulacros m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] Simulacros m)
