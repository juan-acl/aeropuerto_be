...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class ActivacionEmergenciaController : ControllerBase
    {
        ...
        public ActivacionEmergenciaController(IActivacionEmergenciaService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] ActivacionesEmergencia m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ActivacionesEmergencia m)
