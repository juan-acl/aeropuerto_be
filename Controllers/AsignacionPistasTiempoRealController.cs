...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class AsignacionPistasTiempoRealController : ControllerBase
    {
        private readonly IAsignacionPistasTiempoRealService _service;
        public AsignacionPistasTiempoRealController(IAsignacionPistasTiempoRealService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] AsignacionPistasTiempoReal m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] AsignacionPistasTiempoReal m)
