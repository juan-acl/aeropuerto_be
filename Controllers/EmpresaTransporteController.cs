...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class EmpresaTransporteController : ControllerBase
    {
        ...
        public EmpresaTransporteController(IEmpresaTransporteService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] EmpresasTransporte m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] EmpresasTransporte m)
