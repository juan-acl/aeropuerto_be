...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class PoliticaSeguridadController : ControllerBase
    {
        ...
        public PoliticaSeguridadController(IPoliticaSeguridadService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] PoliticasSeguridad m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] PoliticasSeguridad m)
