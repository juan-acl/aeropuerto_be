...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class PlanEmergenciaController : ControllerBase
    {
        ...
        public PlanEmergenciaController(IPlanEmergenciaService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] PlanesEmergencia m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] PlanesEmergencia m)
