...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class RutaTransporteController : ControllerBase
    {
        private readonly IRutaTransporteService _service;
        public RutaTransporteController(IRutaTransporteService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] RutasTransporteTerrestre m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] RutasTransporteTerrestre m)
