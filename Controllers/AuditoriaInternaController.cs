...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class AuditoriaInternaController : ControllerBase
    {
        ...
        public AuditoriaInternaController(IAuditoriaInternaService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] AuditoriasInternas m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] AuditoriasInternas m)
