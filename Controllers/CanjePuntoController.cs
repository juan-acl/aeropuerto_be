...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class CanjePuntoController : ControllerBase
    {
        ...
        public CanjePuntoController(ICanjePuntoService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] CanjesPuntos m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] CanjesPuntos m)
