...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class HuellaCarbonoVueloController : ControllerBase
    {
        ...
        public HuellaCarbonoVueloController(IHuellaCarbonoVueloService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] HuellaCarbonoVuelo m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] HuellaCarbonoVuelo m)
