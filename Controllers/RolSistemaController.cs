...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class RolSistemaController : ControllerBase
    {
        ...
        public RolSistemaController(IRolSistemaService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] RolesSistema m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] RolesSistema m)
