...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class LicenciaOperativaController : ControllerBase
    {
        private readonly ILicenciaOperativaService _service;
        public LicenciaOperativaController(ILicenciaOperativaService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] LicenciasOperativasAeropuerto m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] LicenciasOperativasAeropuerto m)
