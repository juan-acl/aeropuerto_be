...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class EvaluacionPostController : ControllerBase
    {
        private readonly IEvaluacionPostService _service;
        public EvaluacionPostController(IEvaluacionPostService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] EvaluacionesPostEmergencia m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] EvaluacionesPostEmergencia m)
