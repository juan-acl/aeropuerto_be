...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class PedidoCombustibleController : ControllerBase
    {
        private readonly IPedidoCombustibleService _service;
        public PedidoCombustibleController(IPedidoCombustibleService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] PedidosCombustible m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] PedidosCombustible m)
