...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class DocumentoRequeridoController : ControllerBase
    {
        private readonly IDocumentoRequeridoService _service;
        public DocumentoRequeridoController(IDocumentoRequeridoService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] DocumentosRequeridosOperacion m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] DocumentosRequeridosOperacion m)
