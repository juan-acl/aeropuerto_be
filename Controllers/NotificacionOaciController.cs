...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class NotificacionOaciController : ControllerBase
    {
        ...
        public NotificacionOaciController(INotificacionOaciService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] NotificacionesOaci m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] NotificacionesOaci m)
