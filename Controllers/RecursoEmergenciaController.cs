...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class RecursoEmergenciaController : ControllerBase
    {
        ...
        public RecursoEmergenciaController(IRecursoEmergenciaService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] RecursosEmergencia m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] RecursosEmergencia m)
