...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class QuejaTransporteController : ControllerBase
    {
        private readonly IQuejaTransporteService _service;
        public QuejaTransporteController(IQuejaTransporteService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] QuejasTransporteTerrestre m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] QuejasTransporteTerrestre m)
