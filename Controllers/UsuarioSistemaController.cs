...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class UsuarioSistemaController : ControllerBase
    {
        ...
        public UsuarioSistemaController(IUsuarioSistemaService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] UsuariosSistema m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] UsuariosSistema m)
