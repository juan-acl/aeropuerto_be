...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class AuditoriaInternacionalController : ControllerBase
    {
        ...
        public AuditoriaInternacionalController(IAuditoriaInternacionalService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] AuditoriasInternacionales m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] AuditoriasInternacionales m)
