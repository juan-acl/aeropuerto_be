...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class SegmentoClienteController : ControllerBase
    {
        ...
        public SegmentoClienteController(ISegmentoClienteService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] SegmentosClientes m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] SegmentosClientes m)
