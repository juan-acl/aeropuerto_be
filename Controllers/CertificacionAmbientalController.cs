...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class CertificacionAmbientalController : ControllerBase
    {
        private readonly ICertificacionAmbientalService _service;
        public CertificacionAmbientalController(ICertificacionAmbientalService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] CertificacionesAmbientalesAeropuerto m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] CertificacionesAmbientalesAeropuerto m)
