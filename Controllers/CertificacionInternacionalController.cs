...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class CertificacionInternacionalController : ControllerBase
    {
        ...
        public CertificacionInternacionalController(ICertificacionInternacionalService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] CertificacionesInternacionales m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] CertificacionesInternacionales m)
