...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class NewsletterSuscripcionController : ControllerBase
    {
        ...
        public NewsletterSuscripcionController(INewsletterSuscripcionService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] NewsletterSuscripciones m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] NewsletterSuscripciones m)
