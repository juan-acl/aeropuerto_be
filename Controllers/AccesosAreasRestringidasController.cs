...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class AccesosAreasRestringidasController : ControllerBase
    {
        ...
        public AccesosAreasRestringidasController(I service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] TarifasTransporteTerrestre m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] TarifasTransporteTerrestre m)
