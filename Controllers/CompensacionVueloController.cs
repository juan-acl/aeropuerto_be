...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class CompensacionVueloController : ControllerBase
    {
        private readonly ICompensacionVueloService _service;
        public CompensacionVueloController(ICompensacionVueloService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] CompensacionesVuelo m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] CompensacionesVuelo m)
