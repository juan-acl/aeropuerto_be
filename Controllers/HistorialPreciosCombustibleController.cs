...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class HistorialPreciosCombustibleController : ControllerBase
    {
        private readonly IHistorialPreciosCombustibleService _service;
        public HistorialPreciosCombustibleController(IHistorialPreciosCombustibleService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] HistorialPreciosCombustible m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] HistorialPreciosCombustible m)
