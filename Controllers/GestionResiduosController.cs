...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class GestionResiduosController : ControllerBase
    {
        ...
        public GestionResiduosController(IGestionResiduosService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] GestionResiduos m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] GestionResiduos m)
