...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class IncidenteSeguridadController : ControllerBase
    {
        private readonly IIncidenteSeguridadService _service;
        public IncidenteSeguridadController(IIncidenteSeguridadService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] IncidentesSeguridadInformatica m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] IncidentesSeguridadInformatica m)
