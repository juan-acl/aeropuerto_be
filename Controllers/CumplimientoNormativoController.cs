...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class CumplimientoNormativoController : ControllerBase
    {
        ...
        public CumplimientoNormativoController(ICumplimientoNormativoService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] CumplimientoNormativo m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] CumplimientoNormativo m)
