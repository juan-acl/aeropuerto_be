...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class IncidentesInvolucradosController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] IncidentesInvolucradosModel m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] IncidentesInvolucradosModel m)
