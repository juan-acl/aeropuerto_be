...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class AsignacionVehiculoRutaController : ControllerBase
    {
        ...
        public AsignacionVehiculoRutaController(IAsignacionVehiculoRutaService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] AsignacionVehiculosRutas m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] AsignacionVehiculosRutas m)
