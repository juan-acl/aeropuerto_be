...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class ConcesionesComercialesController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] ConcesionesComercialesModel m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ConcesionesComercialesModel m)
