...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class ChoferTransporteController : ControllerBase
    {
        private readonly IChoferTransporteService _service;
        public ChoferTransporteController(IChoferTransporteService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] ChoferesTransporte m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ChoferesTransporte m)
