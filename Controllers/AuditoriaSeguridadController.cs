...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class AuditoriaSeguridadController : ControllerBase
    {
        ...
        public AuditoriaSeguridadController(IAuditoriaSeguridadService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] AuditoriasSeguridad m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] AuditoriasSeguridad m)
