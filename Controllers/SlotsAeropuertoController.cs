...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class SlotsAeropuertoController : ControllerBase
    {
        private readonly ISlotsAeropuertoService _service;
        public SlotsAeropuertoController(ISlotsAeropuertoService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] SlotsAeropuerto m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] SlotsAeropuerto m)
