...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class ProveedorController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] Proveedor m) => Ok(await _service.Insertar(m));
        [HttpGet("{id}")]
        public async Task<IActionResult> GetById(int id) => Ok(await _service.ObtenerPorId(id));
        
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] Proveedor m) => Ok(await _service.Actualizar(m));
