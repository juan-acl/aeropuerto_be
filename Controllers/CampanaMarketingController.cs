...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class CampanaMarketingController : ControllerBase
    {
        ...
        public CampanaMarketingController(ICampanaMarketingService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] CampanasMarketing m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] CampanasMarketing m)
