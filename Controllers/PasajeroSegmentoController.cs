...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class PasajeroSegmentoController : ControllerBase
    {
        ...
        public PasajeroSegmentoController(IPasajeroSegmentoService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] PasajerosSegmentos m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] PasajerosSegmentos m)
