...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class SurtidorCombustibleController : ControllerBase
    {
        ...
        public SurtidorCombustibleController(ISurtidorCombustibleService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] SurtidoresCombustible m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] SurtidoresCombustible m)
