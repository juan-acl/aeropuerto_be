...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class DocumentoImportanteController : ControllerBase
    {
        private readonly IDocumentoImportanteService _service;
        public DocumentoImportanteController(IDocumentoImportanteService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] DocumentosImportantes m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] DocumentosImportantes m)
