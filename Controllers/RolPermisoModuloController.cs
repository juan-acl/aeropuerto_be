...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class RolPermisoModuloController : ControllerBase
    {
        ...
        public RolPermisoModuloController(IRolPermisoModuloService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] RolesPermisosModulos m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] RolesPermisosModulos m)
