...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class ReservaTransporteController : ControllerBase
    {
        private readonly IReservaTransporteService _service;
        public ReservaTransporteController(IReservaTransporteService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] ReservasTransporteTerrestre m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ReservasTransporteTerrestre m)
