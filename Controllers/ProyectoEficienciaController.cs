...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class ProyectoEficienciaController : ControllerBase
    {
        private readonly IProyectoEficienciaService _service;
        public ProyectoEficienciaController(IProyectoEficienciaService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] ProyectosEficienciaEnergetica m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ProyectosEficienciaEnergetica m)
