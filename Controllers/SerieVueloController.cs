...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class SerieVueloController : ControllerBase
    {
        private readonly ISerieVueloService _service;
        public SerieVueloController(ISerieVueloService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] SeriesVueloAsignadas m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] SeriesVueloAsignadas m)
