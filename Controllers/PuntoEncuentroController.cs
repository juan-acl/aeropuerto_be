...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class PuntoEncuentroController : ControllerBase
    {
        ...
        public PuntoEncuentroController(IPuntoEncuentroService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] PuntosEncuentro m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] PuntosEncuentro m)
