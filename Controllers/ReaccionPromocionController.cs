...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class ReaccionPromocionController : ControllerBase
    {
        ...
        public ReaccionPromocionController(IReaccionPromocionService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] ReaccionesPromociones m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ReaccionesPromociones m)
