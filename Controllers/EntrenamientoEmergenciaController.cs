...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class EntrenamientoEmergenciaController : ControllerBase
    {
        ...
        public EntrenamientoEmergenciaController(IEntrenamientoEmergenciaService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] EntrenamientosEmergencia m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] EntrenamientosEmergencia m)
