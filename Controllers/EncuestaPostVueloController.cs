...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class EncuestaPostVueloController : ControllerBase
    {
        ...
        public EncuestaPostVueloController(IEncuestaPostVueloService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] EncuestasPostVuelo m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] EncuestasPostVuelo m)
