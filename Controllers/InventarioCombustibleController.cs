...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class InventarioCombustibleController : ControllerBase
    {
        ...
        public InventarioCombustibleController(IInventarioCombustibleService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] InventarioCombustible m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] InventarioCombustible m)
