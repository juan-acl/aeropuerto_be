...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class FacturacionCombustibleController : ControllerBase
    {
        ...
        public FacturacionCombustibleController(IFacturacionCombustibleService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] FacturacionCombustible m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] FacturacionCombustible m)
