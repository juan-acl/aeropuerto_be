...
namespace Aeropuerto.Backend.Controllers
{
    ...
    [ApiController]
    public class ConvenioHotelController : ControllerBase
    {
        private readonly IConvenioHotelService _service;
        public ConvenioHotelController(IConvenioHotelService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] ConveniosHotelesTransporte m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ConveniosHotelesTransporte m)
