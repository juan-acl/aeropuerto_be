...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class ControlCalidadCombustibleController : ControllerBase
    {
        ...
        public ControlCalidadCombustibleController(IControlCalidadCombustibleService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] ControlCalidadCombustible m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ControlCalidadCombustible m)
