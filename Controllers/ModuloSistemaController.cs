...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class ModuloSistemaController : ControllerBase
    {
        ...
        public ModuloSistemaController(IModuloSistemaService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] ModulosSistema m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ModulosSistema m)
