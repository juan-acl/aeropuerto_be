...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class NotificacionLegalController : ControllerBase
    {
        ...
        public NotificacionLegalController(INotificacionLegalService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] NotificacionesLegales m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] NotificacionesLegales m)
