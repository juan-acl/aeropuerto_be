...
{
    [ApiController]
    [Route("api/retrasosvuelo")]
    public class RetrasoVueloController : ControllerBase
    {
        private readonly IRetrasoVueloService _service;
        public RetrasoVueloController(IRetrasoVueloService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Crear([FromBody] RetrasoVueloModel modelo)
        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Actualizar(int id, [FromBody] RetrasoVueloModel modelo)
        {
            try {
