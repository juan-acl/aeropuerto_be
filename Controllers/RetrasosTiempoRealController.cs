...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class RetrasosTiempoRealController : ControllerBase
    {
        ...
        public RetrasosTiempoRealController(IRetrasosTiempoRealService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] RetrasosTiempoReal m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] RetrasosTiempoReal m)
