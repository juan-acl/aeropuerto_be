...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class CuentaBancariaController : ControllerBase
    {
        ...
        public CuentaBancariaController(ICuentaBancariaService service) => _service = service;

        [HttpGet]
        public async Task<IActionResult> Get() => Ok(await _service.ListarTodo());

        [HttpPost]
        public async Task<IActionResult> Post([FromBody] CuentaBancaria m) => Ok(await _service.Insertar(m));
        [HttpGet("{id}")]
        public async Task<IActionResult> GetById(int id) => Ok(await _service.ObtenerPorId(id));
        
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] CuentaBancaria m) => Ok(await _service.Actualizar(m));
