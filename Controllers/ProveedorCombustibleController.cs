...
namespace Aeropuerto.Backend.Controllers
{
    ...
    public class ProveedorCombustibleController : ControllerBase
    {
        ...
        public ProveedorCombustibleController(IProveedorCombustibleService service) => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] ProveedoresCombustible m)
            => await _service.Insertar(m) ? Ok(new { mensaje = "Creado" }) : BadRequest(new { mensaje = "Error al crear" });

        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ProveedoresCombustible m)
