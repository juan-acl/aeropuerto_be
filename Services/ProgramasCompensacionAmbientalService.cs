...
{
    public class ProgramasCompensacionAmbientalService : IProgramasCompensacionAmbientalService
    {
        private readonly DBContext _primary;
        private readonly ReplicaDBContext _replica;
        public ProgramasCompensacionAmbientalService(DBContext primary, ReplicaDBContext replica) { _primary = primary; _replica = replica; }

        public async Task<List<ProgramasCompensacionAmbiental>> ListarTodo()
        {
            try { return await _replica.ProgramasCompensacionAmbiental.ToListAsync(); }
        ...
        {
            try { return await _replica.ProgramasCompensacionAmbiental.FindAsync(id); }
        ...
        public async Task<bool> Insertar(ProgramasCompensacionAmbiental m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Insertar ProgramasCompensacionAmbiental: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Actualizar(int id, ProgramasCompensacionAmbiental m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Actualizar ProgramasCompensacionAmbiental: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Eliminar(int id)
        {
            try
            {
                string sql = "BEGIN pkg_programas_compensacion.delete_programa(:p_id); END;";
                await _primary.Database.ExecuteSqlRawAsync(sql, new OracleParameter("p_id", id));
                return true;
            }
            catch (Exception ex) { Console.WriteLine($"ERROR Eliminar ProgramasCompensacionAmbiental: {ex.Message}"); throw; }
        }
    }
}
