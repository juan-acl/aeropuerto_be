...
namespace Aeropuerto.Backend.Services
{
    public class ObjetosEntregadosService : IObjetosEntregadosService
    {
        ...
        public ObjetosEntregadosService(DBContext primary, ReplicaDBContext replica) { _primary = primary; _replica = replica; }

        public async Task<List<ObjetosEntregadosModel>> ListarTodo()
        {
            try { return await _replica.ObjetosEntregados.ToListAsync(); }
        ...
        {
            try { return await _replica.ObjetosEntregados.FindAsync(id); }
        ...
        public async Task<bool> Insertar(ObjetosEntregadosModel m)
        {
            try
            {
                ...
                var p = new OracleParameter[] {
                    ...
                    new OracleParameter("p_firma_digital", (object?)m.FirmaDigital ?? DBNull.Value),
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Insertar ObjetosEntregadosModel: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Actualizar(int id, ObjetosEntregadosModel m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Actualizar ObjetosEntregadosModel: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Eliminar(int id)
        {
            try
            {
                string sql = "BEGIN pkg_objetos_entregados.delete_entrega(:p_id); END;";
                await _primary.Database.ExecuteSqlRawAsync(sql, new OracleParameter("p_id", id));
                return true;
            }
            catch (Exception ex) { Console.WriteLine($"ERROR Eliminar ObjetosEntregadosModel: {ex.Message}"); throw; }
        }
    }
}
