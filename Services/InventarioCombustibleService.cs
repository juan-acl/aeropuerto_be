...
namespace Aeropuerto.Backend.Services
{
    public class InventarioCombustibleService : IInventarioCombustibleService
    {
        ...
        public InventarioCombustibleService(DBContext primary, ReplicaDBContext replica) { _primary = primary; _replica = replica; }

        public async Task<List<InventarioCombustible>> ListarTodo()
        {
            try { return await _replica.InventarioCombustible.ToListAsync(); }
        ...
        {
            try { return await _replica.InventarioCombustible.FindAsync(id); }
        ...
        public async Task<bool> Insertar(InventarioCombustible m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Insertar InventarioCombustible: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Actualizar(int id, InventarioCombustible m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Actualizar InventarioCombustible: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Eliminar(int id)
        {
            try
            {
                string sql = "BEGIN pkg_inventario_combustible.delete_inventario(:p_id); END;";
                await _primary.Database.ExecuteSqlRawAsync(sql, new OracleParameter("p_id", id));
                return true;
            }
            catch (Exception ex) { Console.WriteLine($"ERROR Eliminar InventarioCombustible: {ex.Message}"); throw; }
        }
    }
}
