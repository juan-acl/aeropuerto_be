...
namespace Aeropuerto.Backend.Services
{
    public class PlanEmergenciaService : IPlanEmergenciaService
    {
        ...
        {
            try { return await _context.PlanesEmergencia.ToListAsync(); }
        ...
        {
            try { return await _context.PlanesEmergencia.FindAsync(id); }
        ...
        public async Task<bool> Insertar(PlanesEmergencia m)
        {
            try
            {
                ...
                new OracleParameter("p_tiempo_respuesta_estimado", DBNull.Value /* TiempoRespuestaEstimado */),
                ...
                new OracleParameter("p_documento_plan", OracleDbType.Blob) { Value = (object?)m.DocumentoPlan ?? DBNull.Value },
                new OracleParameter("p_activo", (object?)m.Activo ?? DBNull.Value)
                };
                await _context.Database.ExecuteSqlRawAsync(sql, p);
                return true;
            }
            catch (Exception ex) { Console.WriteLine($"ERROR Insertar PlanesEmergencia: {ex.Message}"); return false; }
        }
        ...
        public async Task<bool> Actualizar(int id, PlanesEmergencia m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Actualizar PlanesEmergencia: {ex.Message}"); return false; }
        }
        ...
        public async Task<bool> Eliminar(int id)
        {
            try
            {
                string sql = "BEGIN pkg_planes_emergencia.delete_plan(:); END;";
                await _context.Database.ExecuteSqlRawAsync(sql, new OracleParameter("", id));
                return true;
            }
            catch (Exception ex) { Console.WriteLine($"ERROR Eliminar PlanesEmergencia: {ex.Message}"); return false; }
        }
    }
}
