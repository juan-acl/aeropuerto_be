...
{
    public class CertificacionInternacionalService : ICertificacionInternacionalService
    {
        private readonly DBContext _context;
        public CertificacionInternacionalService(DBContext context) => _context = context;
        ...
        public async Task<bool> Insertar(CertificacionesInternacionales m)
        {
            ...
            await _context.Database.ExecuteSqlRawAsync("BEGIN pkg_certificaciones_internacionales.insert_certificacion(:p_id_estandar, :p_nombre_certificacion, :p_organismo_certificador, :p_fecha_emision, :p_fecha_vencimiento, :p_alcance_certificacion, :p_numero_certificado, :p_documento_certificado, :p_activa, :p_responsable_seguimiento); END;", p);
            return true;
        }
        ...
        public async Task<bool> Actualizar(int id, CertificacionesInternacionales m)
        {
            var p = new List<OracleParameter> {
                new OracleParameter("p_id_certificacion_internacional", m.IdCertificacionInternacional)
            };
            p.AddRange(new[] {
                new OracleParameter("p_id_estandar", (object?)m.IdEstandar ?? DBNull.Value),
                new OracleParameter("p_nombre_certificacion", (object?)m.NombreCertificacion ?? DBNull.Value),
            ...
            await _context.Database.ExecuteSqlRawAsync("BEGIN pkg_certificaciones_internacionales.update_certificacion(:p_id_certificacion_internacional, :p_id_estandar, :p_nombre_certificacion, :p_organismo_certificador, :p_fecha_emision, :p_fecha_vencimiento, :p_alcance_certificacion, :p_numero_certificado, :p_documento_certificado, :p_activa, :p_responsable_seguimiento); END;", p.ToArray());
            return true;
        }

        public async Task<bool> Eliminar(int id)
        {
            await _context.Database.ExecuteSqlRawAsync("BEGIN pkg_certificaciones_internacionales.delete_certificacion(:p_id_certificacion_internacional); END;", 
                new OracleParameter("p_id_certificacion_internacional", id));
            return true;
        }

        public async Task<List<CertificacionesInternacionales>> ListarTodo() => await _context.Set<CertificacionesInternacionales>().ToListAsync();

        public async Task<CertificacionesInternacionales?> ObtenerPorId(int id) => await _context.Set<CertificacionesInternacionales>().FindAsync(id);
    }
}
