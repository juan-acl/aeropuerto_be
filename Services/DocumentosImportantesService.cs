...
{
    public class DocumentosImportantesService : IDocumentosImportantesService
    {
        private readonly DBContext _primary;
        private readonly ReplicaDBContext _replica;
        public DocumentosImportantesService(DBContext primary, ReplicaDBContext replica) { _primary = primary; _replica = replica; }

        public async Task<List<DocumentosImportantes>> ListarTodo()
        {
            try { return await _replica.DocumentosImportantes.ToListAsync(); }
        ...
        {
            try { return await _replica.DocumentosImportantes.FindAsync(id); }
        ...
        public async Task<bool> Insertar(DocumentosImportantes m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Insertar DocumentosImportantes: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Actualizar(int id, DocumentosImportantes m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Actualizar DocumentosImportantes: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Eliminar(int id)
        {
            try
            {
                string sql = "BEGIN pkg_documentos_importantes.delete_documento(:p_id); END;";
                await _primary.Database.ExecuteSqlRawAsync(sql, new OracleParameter("p_id", id));
                return true;
            }
            catch (Exception ex) { Console.WriteLine($"ERROR Eliminar DocumentosImportantes: {ex.Message}"); throw; }
        }
    }
}
