...
namespace Aeropuerto.Backend.Services
{
    public class EquipajeEspecialService : IEquipajeEspecialService
    {
        ...
        public EquipajeEspecialService(DBContext primary, ReplicaDBContext replica) { _primary = primary; _replica = replica; }

        public async Task<List<EquipajeEspecialModel>> ListarTodo()
        {
            try { return await _replica.EquipajeEspecial.ToListAsync(); }
        ...
        {
            try { return await _replica.EquipajeEspecial.FindAsync(id); }
        ...
        public async Task<bool> Insertar(EquipajeEspecialModel m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Insertar EquipajeEspecialModel: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Actualizar(int id, EquipajeEspecialModel m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Actualizar EquipajeEspecialModel: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Eliminar(int id)
        {
            try
            {
                string sql = "BEGIN pkg_equipaje_especial.delete_equipaje(:p_id); END;";
                await _primary.Database.ExecuteSqlRawAsync(sql, new OracleParameter("p_id", id));
                return true;
            }
            catch (Exception ex) { Console.WriteLine($"ERROR Eliminar EquipajeEspecialModel: {ex.Message}"); throw; }
        }
    }
}
