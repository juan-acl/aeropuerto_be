...
namespace Aeropuerto.Backend.Services
{
    public class EmergenciasMedicasService : IEmergenciasMedicasService
    {
        ...
        public EmergenciasMedicasService(DBContext primary, ReplicaDBContext replica) { _primary = primary; _replica = replica; }

        public async Task<List<EmergenciasMedicasModel>> ListarTodo()
        {
            try { return await _replica.EmergenciasMedicas.ToListAsync(); }
        ...
        {
            try { return await _replica.EmergenciasMedicas.FindAsync(id); }
        ...
        public async Task<bool> Insertar(EmergenciasMedicasModel m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Insertar EmergenciasMedicasModel: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Actualizar(int id, EmergenciasMedicasModel m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Actualizar EmergenciasMedicasModel: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Eliminar(int id)
        {
            try
            {
                string sql = "BEGIN pkg_emergencias_medicas.delete_emergencia(:p_id); END;";
                await _primary.Database.ExecuteSqlRawAsync(sql, new OracleParameter("p_id", id));
                return true;
            }
            catch (Exception ex) { Console.WriteLine($"ERROR Eliminar EmergenciasMedicasModel: {ex.Message}"); throw; }
        }
    }
}
