...
namespace Aeropuerto.Backend.Services
{
    public class PerfilViajeroService : IPerfilViajeroService
    {
        ...
        public PerfilViajeroService(DBContext primary, ReplicaDBContext replica) { _primary = primary; _replica = replica; }

        public async Task<List<PerfilViajeroModel>> ListarTodo()
        {
            try { return await _replica.PerfilesViajero.ToListAsync(); }
        ...
        {
            try { return await _replica.PerfilesViajero.FindAsync(id); }
        ...
        public async Task<bool> Insertar(PerfilViajeroModel m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Insertar PerfilViajeroModel: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Actualizar(int id, PerfilViajeroModel m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Actualizar PerfilViajeroModel: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Eliminar(int id)
        {
            try
            {
                string sql = "BEGIN pkg_perfiles_viajero.delete_perfil(:p_id); END;";
                await _primary.Database.ExecuteSqlRawAsync(sql, new OracleParameter("p_id", id));
                return true;
            }
            catch (Exception ex) { Console.WriteLine($"ERROR Eliminar PerfilViajeroModel: {ex.Message}"); throw; }
        }
    }
}
