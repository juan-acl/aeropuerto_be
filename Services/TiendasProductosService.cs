...
namespace Aeropuerto.Backend.Services
{
    public class TiendasProductosService : ITiendasProductosService
    {
        ...
        public TiendasProductosService(DBContext primary, ReplicaDBContext replica) { _primary = primary; _replica = replica; }

        public async Task<List<TiendasProductosModel>> ListarTodo()
        {
            try { return await _replica.TiendasProductos.ToListAsync(); }
        ...
        {
            try { return await _replica.TiendasProductos.FindAsync(id); }
        ...
        public async Task<bool> Insertar(TiendasProductosModel m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Insertar TiendasProductosModel: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Actualizar(int id, TiendasProductosModel m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Actualizar TiendasProductosModel: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Eliminar(int id)
        {
            try
            {
                string sql = "BEGIN pkg_tiendas_productos.delete_producto(:p_id); END;";
                await _primary.Database.ExecuteSqlRawAsync(sql, new OracleParameter("p_id", id));
                return true;
            }
            catch (Exception ex) { Console.WriteLine($"ERROR Eliminar TiendasProductosModel: {ex.Message}"); throw; }
        }
    }
}
