...
namespace Aeropuerto.Backend.Services
{
    public class IncidentesInvolucradosService : IIncidentesInvolucradosService
    {
        ...
        public IncidentesInvolucradosService(DBContext primary, ReplicaDBContext replica) { _primary = primary; _replica = replica; }

        public async Task<List<IncidentesInvolucradosModel>> ListarTodo()
        {
            try { return await _replica.IncidentesInvolucrados.ToListAsync(); }
        ...
        {
            try { return await _replica.IncidentesInvolucrados.FindAsync(id); }
        ...
        public async Task<bool> Insertar(IncidentesInvolucradosModel m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Insertar IncidentesInvolucradosModel: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Actualizar(int id, IncidentesInvolucradosModel m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Actualizar IncidentesInvolucradosModel: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Eliminar(int id)
        {
            try
            {
                string sql = "BEGIN pkg_incidentes_involucrados.delete_involucrado(:p_id); END;";
                await _primary.Database.ExecuteSqlRawAsync(sql, new OracleParameter("p_id", id));
                return true;
            }
            catch (Exception ex) { Console.WriteLine($"ERROR Eliminar IncidentesInvolucradosModel: {ex.Message}"); throw; }
        }
    }
}
