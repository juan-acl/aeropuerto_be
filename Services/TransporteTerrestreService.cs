...
namespace Aeropuerto.Backend.Services
{
    public class TransporteTerrestreService : ITransporteTerrestreService
    {
        ...
        public TransporteTerrestreService(DBContext primary, ReplicaDBContext replica) { _primary = primary; _replica = replica; }

        public async Task<List<TransporteTerrestreModel>> ListarTodo()
        {
            try { return await _replica.TransporteTerrestre.ToListAsync(); }
        ...
        {
            try { return await _replica.TransporteTerrestre.FindAsync(id); }
        ...
        public async Task<bool> Insertar(TransporteTerrestreModel m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Insertar TransporteTerrestreModel: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Actualizar(int id, TransporteTerrestreModel m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Actualizar TransporteTerrestreModel: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Eliminar(int id)
        {
            try
            {
                string sql = "BEGIN pkg_transporte_terrestre.delete_transporte(:p_id); END;";
                await _primary.Database.ExecuteSqlRawAsync(sql, new OracleParameter("p_id", id));
                return true;
            }
            catch (Exception ex) { Console.WriteLine($"ERROR Eliminar TransporteTerrestreModel: {ex.Message}"); throw; }
        }
    }
}
