...
namespace Aeropuerto.Backend.Services
{
    public class AuditoriaInternaService : IAuditoriaInternaService
    {
        ...
        public async Task<bool> Insertar(AuditoriasInternas m)
        {
            ...
            await _context.Database.ExecuteSqlRawAsync("BEGIN pkg_auditorias_internas.insert_auditoria(:p_codigo_auditoria, :p_titulo, :p_tipo_auditoria, :p_alcance, :p_fecha_inicio_planeacion, :p_fecha_fin_planeacion, :p_fecha_inicio_ejecucion, :p_fecha_fin_ejecucion, :p_fecha_informe, :p_auditor_lider, :p_equipo_auditor, :p_areas_auditadas, :p_hallazgos, :p_no_conformidades, :p_oportunidades_mejora, :p_conclusiones, :p_informe_final, :p_estado); END;", p);
            return true;
        }
        ...
        public async Task<bool> Actualizar(int id, AuditoriasInternas m)
        {
            var p = new List<OracleParameter> {
                new OracleParameter("p_id_auditoria_interna", m.IdAuditoriaInterna)
            };
            p.AddRange(new[] {
                new OracleParameter("p_codigo_auditoria", (object?)m.CodigoAuditoria ?? DBNull.Value),
                new OracleParameter("p_titulo", (object?)m.Titulo ?? DBNull.Value),
            ...
            await _context.Database.ExecuteSqlRawAsync("BEGIN pkg_auditorias_internas.update_auditoria(:p_id_auditoria_interna, :p_codigo_auditoria, :p_titulo, :p_tipo_auditoria, :p_alcance, :p_fecha_inicio_planeacion, :p_fecha_fin_planeacion, :p_fecha_inicio_ejecucion, :p_fecha_fin_ejecucion, :p_fecha_informe, :p_auditor_lider, :p_equipo_auditor, :p_areas_auditadas, :p_hallazgos, :p_no_conformidades, :p_oportunidades_mejora, :p_conclusiones, :p_informe_final, :p_estado); END;", p.ToArray());
            return true;
        }

        public async Task<bool> Eliminar(int id)
        {
            await _context.Database.ExecuteSqlRawAsync("BEGIN pkg_auditorias_internas.delete_auditoria(:p_id_auditoria_interna); END;", 
                new OracleParameter("p_id_auditoria_interna", id));
            return true;
        }

        public async Task<List<AuditoriasInternas>> ListarTodo() => await _context.Set<AuditoriasInternas>().ToListAsync();

        public async Task<AuditoriasInternas?> ObtenerPorId(int id) => await _context.Set<AuditoriasInternas>().FindAsync(id);
    }
}
