...
namespace Aeropuerto.Backend.Services
{
    public class TarifasEspecialesService : ITarifasEspecialesService
    {
        ...
        public TarifasEspecialesService(DBContext primary, ReplicaDBContext replica) { _primary = primary; _replica = replica; }

        public async Task<List<TarifasEspecialesModel>> ListarTodo()
        {
            try { return await _replica.TarifasEspeciales.ToListAsync(); }
        ...
        {
            try { return await _replica.TarifasEspeciales.FindAsync(id); }
        ...
        public async Task<bool> Insertar(TarifasEspecialesModel m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Insertar TarifasEspecialesModel: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Actualizar(int id, TarifasEspecialesModel m)
        {
            ...
            catch (Exception ex) { Console.WriteLine($"ERROR Actualizar TarifasEspecialesModel: {ex.Message}"); throw; }
        }
        ...
        public async Task<bool> Eliminar(int id)
        {
            try
            {
                string sql = "BEGIN pkg_tarifas_especiales.delete_tarifa(:p_id); END;";
                await _primary.Database.ExecuteSqlRawAsync(sql, new OracleParameter("p_id", id));
                return true;
            }
            catch (Exception ex) { Console.WriteLine($"ERROR Eliminar TarifasEspecialesModel: {ex.Message}"); throw; }
        }
    }
}
